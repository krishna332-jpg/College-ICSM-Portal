...
// 3. Database Fetch - Sort by deadline (Active first, then expired)
$current_date = date('Y-m-d');
$query = "SELECT *, 
          CASE WHEN deadline < '$current_date' THEN 1 ELSE 0 END AS is_expired 
          FROM opportunities 
          WHERE type = 'Certification' AND status = 'active' 
          ORDER BY is_expired ASC, id DESC";
...
<head>
    ...
    <title>Professional Certifications | ICSM Portal</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Inter', sans-serif; }
        ...
        .hero-img-box {
            ...
            background: url('../images/certi.jpg') no-repeat center center;
        ...
        .org-tag { 
            ...
            color: var(--primary); background: #f5f3ff; padding: 5px 12px;
...
<section class="hero-container">
    <div class="hero-text">
        ...
        <p>Upgrade your skills with globally recognized certification programs from leading platforms.</p>
        <div class="search-wrapper">
            <i class="fa-solid fa-magnifying-glass"></i>
            <input type="text" id="certSearch" placeholder="Search courses or vendors..." onkeyup="filterCerts()">
        </div>
    </div>
...
<div class="container">
    <div class="opp-grid" id="certGrid">
        <?php if(mysqli_num_rows($result) > 0): ?>
            <?php while($row = mysqli_fetch_assoc($result)):
                $op_id = $row['id'];
                $deadline_date = $row['deadline'];
                $is_expired = ($deadline_date < $current_date);
                
                // CRITICAL CHECK: Does an application exist for this student and this opportunity?
                $check_sql = "SELECT id FROM applications WHERE (student_id = '$student_id' OR student_id = '$reg_no') AND opportunity_id = '$op_id'";
                $check_app = mysqli_query($conn, $check_sql);
                $has_applied = mysqli_num_rows($check_app) > 0;
            ?>
                <div class="card <?php echo $is_expired ? 'expired' : ''; ?>">
                    <div class="card-banner" style="background-image: url('../<?php echo $row['image']; ?>'), url('../images/default_cert.jpg');"></div>
                    <div class="card-body">
                        ...
                        <div class="card-footer">
                            ...
                            <a href="view_opportunity.php?id=<?php echo $row['id']; ?>" class="btn-view">
                                <?php 
                                    ...
                                    else echo $has_applied ? "View Status" : "Enroll Now"; 
                                ?>
                            </a>
                        </div>
                    </div>
            ...
            <?php endwhile; ?>
        <?php else: ?>
            <div style="grid-column: span 3; text-align: center; padding: 100px 0; color: #94a3b8;">
                <i class="fa-solid fa-graduation-cap" style="font-size: 50px; margin-bottom: 15px; display: block; opacity: 0.3;"></i>
                <p style="font-weight: 600;">No certifications currently listed.</p>
            </div>
        <?php endif; ?>
    </div>
</div>
...
function filterCerts() {
    let input = document.getElementById('certSearch').value.toLowerCase();
